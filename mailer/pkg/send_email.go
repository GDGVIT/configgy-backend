...
type MailInstance struct {
	...
	smtpHost     string `validate:"required,ip|hostname"`
	smtpPort     int    `validate:"required,numeric,oneof=25 465 587 2525"`
}
type Message struct {
	From         string
	To           []string
...
func (svc *MailInstance) SetCredentials(username string, password string) error {
	...
	svc.smtpUsername = username

	if err := validateVar(password, "required"); err != nil {
		return err
	}
	svc.smtpPassword = password
	return nil
}
...
func (svc *MailInstance) SendEmail(msg Message) error {
	...
	host := svc.smtpHost
	port := svc.smtpPort
	username := svc.smtpUsername
	password := svc.smtpPassword
...
func (svc *MailInstance) GetTransportDetails() (string, int) {
	return svc.smtpHost, svc.smtpPort
}
...
func (svc *MailInstance) GetCredentials() (string, string) {
	return svc.smtpUsername, svc.smtpPassword
}
